...
let postRegister = async (req, res) => { // post emailUser to the database before check emailUser exists or not 
  ...
  try {
   let createUserSuccsess = await auth.register(req.body.email, req.body.gender, req.body.password) // register taken from file && await check emailUser exists or not  authServices
